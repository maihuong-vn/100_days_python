...
while True:     #True means loop will run until we break it, alternative: "game_over = False, while not game_over:"
    ...
    display = ""
    for letter in chosen_word:              #condition for correct guess to display, like later:  if guess in chosen_word:print("Good guess!") guessed_letter.add(guess) 
        if letter in guessed_letter:      #keep the correct letters revealed
            display += letter                 ##if guessed letter is correct, add it to display, (e.g. b_____), no need to show hangman art 
        else:
            display += "_"
    print(display)
    ...
    if guess in display or guess in wrong_guesses:       # neu guess truoc dc doan dung aka dc displayed hoac sai truoc do
    ...
    if guess in chosen_word:
        print("Good guess!")
        guessed_letter.add(guess)        #add correct guessed letter to the guessed_letter set
